...
public class Juego {
    ...
    /**
     * Atributo para recordar en que sala estas.
     */
    ...
    private void combatir(Personaje enemigo) {
        ...
        while (jugador.estaVivo() && enemigo.estaVivo()) {
            ...
            } // Pausa dramática para el enemigo
    ...
    /**
     * Metodo para guardar la partida actual en un archivo de texto llamado "partida.txt".
     *
     */
    private void guardarPartida() {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("partida.txt"))) {
            ...
            if (jugador instanceof Guerrero) {
                ...
                bw.write("MAGO");
            } else if (jugador instanceof Picaro) {
                bw.write("PICARO");
            } else {
                System.out.println("Clase de personaje no reconocida.");
                return;
            }
            bw.newLine();

            bw.write(String.valueOf(jugador.getNivel()));
            bw.newLine();

            bw.write(String.valueOf(jugador.getPuntosVida()));
            bw.newLine();

            bw.write(String.valueOf(this.nivelActual));
            bw.newLine();
    ...
    /**
     * Metodo para cargar particas guardadas desde el archivo "partida.txt".
     *
     */
    private void cargarPartida() {
        try (BufferedReader br = new BufferedReader(new FileReader("partida.txt"))) {
            ...
            int nivelGuardado = Integer.parseInt(br.readLine());
            double vida = Double.parseDouble(br.readLine());
            int nivelSalaGuardado = Integer.parseInt(br.readLine());

            if (nombre == null || clase == null) {
                System.out.println("El archivo de guardado está incompleto.");
                return;
            }

            if (clase.equals("GUERRERO")) {
                jugador = new Guerrero(nombre, new Arma("Espada de Madera", 8));
            } else if (clase.equals("MAGO")) {
                jugador = new Mago(nombre);
            } else if (clase.equals("PICARO")) {
                jugador = new Picaro(nombre, new Arma("Daga Lamentable", 4), new Arma("Daga Oxidada", 4));
            } else {
                System.out.println("La clase guardada no es válida.");
                return;
            }
            ...
            jugador.setPuntosVida(vida);

            this.gestorMisiones = new GestorMisiones(jugador);
            this.nivelActual = nivelSalaGuardado;
            cargarNivel(this.nivelActual);
